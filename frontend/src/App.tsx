...
export default function App() {
  ...
  const getInitialUser = (): UserProfile => {
    const saved = localStorage.getItem('forensiq_user');
    if (saved) {
      try {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(() => !!localStorage.getItem('forensiq_user'));
  ...
  const handleRegisterSuccess = (profile: UserProfile) => {
    ...
    setCurrentTab('home');
    localStorage.setItem('forensiq_user', JSON.stringify(newUser));
  };
  ...
  const handleMockSignIn = () => {
    ...
    setIsAuthenticated(true);
    setCurrentTab('home');
    localStorage.setItem('forensiq_user', JSON.stringify(mockUser));
  };

  const handleLogout = () => {
    setIsAuthenticated(false);
    localStorage.removeItem('forensiq_user');
  };
