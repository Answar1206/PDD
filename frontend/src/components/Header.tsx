...
export type TabType = 'home' | 'dashboard' | 'text' | 'pdf' | 'image' | 'video' | 'settings' | 'how-it-works';
    ...
    <header className="fixed top-0 w-full z-50 flex justify-between items-center px-4 md:px-8 h-16 bg-white/80 backdrop-blur-lg border-b border-black/5 text-[#300000] select-none transition-colors duration-300">
      ...
      <nav className="hidden md:flex items-center gap-6 lg:gap-10 font-sans text-[13px] font-bold uppercase tracking-wider">
        ...
        <button 
          ...
          className={`hover:text-[#800000] transition-colors relative pb-1 ${currentTab === 'dashboard' ? 'text-[#800000]' : 'text-[#300000]/60'}`}
        >
          Dashboard
          {currentTab === 'dashboard' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#800000] rounded-t-full"></div>}
        </button>
        <button 
